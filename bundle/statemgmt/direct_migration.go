...
func convertTFStateToDirect(ctx context.Context, b *bundle.Bundle, tfState *migrate.TFState) (string, int, bool, error) {
	...
	if logdiag.HasError(ctx) {
		return tempStatePath, resourceCount, false, safeerr.New("failed to apply secret scope fixups")
	}
	...
	if err != nil {
		return tempStatePath, resourceCount, false, safeerr.Errorf("failed to reverse interpolation: %w", err)
	}
	...
	if err != nil {
		return tempStatePath, resourceCount, false, safeerr.Errorf("failed to create uninterpolated config: %w", err)
	}
	...
	if err := stateDB.UpgradeToWrite(); err != nil {
		return tempStatePath, resourceCount, false, safeerr.Errorf("upgrading state for apply: %w", err)
	}
	...
	if logdiag.HasError(ctx) {
		return tempStatePath, resourceCount, hasWarnings, safeerr.New("state conversion failed")
	}
